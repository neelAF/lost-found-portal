...
export function ReportFoundForm() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden px-5 py-8 sm:px-8 lg:px-10">
      <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_top_right,_rgba(34,197,94,0.25),_transparent_28%),radial-gradient(circle_at_bottom_left,_rgba(59,130,246,0.2),_transparent_26%),linear-gradient(180deg,_#f6fffb,_#effcf6_55%,_#f8fafc)]" />
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-5">
            ...
            <div>
              ...
              <input
                ...
                className="w-full rounded-[1.35rem] border border-white/45 bg-white/60 px-4 py-3 text-sm text-slate-700 shadow-inner shadow-white/60 outline-none file:mr-4 file:rounded-full file:border-0 file:bg-emerald-100 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-emerald-700 dark:bg-white/10 dark:text-slate-200"
              />
            </div>
            ...
            <div className="flex flex-col gap-3 pt-2 sm:flex-row sm:items-center sm:justify-end">
              ...
              <button
                type="submit"
                disabled={isPending}
                className="inline-flex min-h-13 flex-1 items-center justify-center rounded-[1.35rem] bg-emerald-600 px-6 py-4 text-sm font-semibold text-white shadow-lg shadow-emerald-500/25 transition hover:bg-emerald-700 hover:shadow-emerald-500/35 disabled:cursor-not-allowed disabled:opacity-70"
              >
                {isPending ? "Submitting..." : "Submit found item report"}
              </button>
            </div>
          </form>
        </div>
